...
const submit = (event: FormSubmitEvent<z.output<typeof schema>>) => {
  ...
  return $fetch('/api/user', {
    ...
    .then(() => {
      toast.add({ title: 'Succès', description: 'Account updated', color: 'green' })
    })
...
<template>
  <UCard>
    <template #header>
      <p class="text-2xl">Update your account</p>
    </template>
    ...
    <UForm ref="form" :schema="schema" :state="state" @submit="submit">
      ...
      <UButton :loading="loading" class="mt-4" type="submit">Update</UButton>
    </UForm>
  </UCard>
</template>
