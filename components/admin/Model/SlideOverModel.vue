...
const submit = (event: FormSubmitEvent<z.output<typeof props.schema>>) => {
  useFormSubmit(
    () => {
      toast.add({ title: 'Success', description: props.successToastMessage, color: 'green' })
  ...
  <USlideover v-model="isOpen">
    ...
    >
      ...
      <UForm :ref="form" :state="state" :schema="schema" class="space-y-4" @submit="submit">
        <slot />
        <UButton :loading="loading" class="mt-4" type="submit" :color="buttonColor ? buttonColor : 'royal-purple'">{{
          submitButtonText
        }}</UButton>
      </UForm>
    </UCard>
